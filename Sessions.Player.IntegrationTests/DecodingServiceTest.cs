...
namespace Sessions.Player.Tests
{
	...
	public class DecodingServiceTest
	{
		...
		public void TestDecode(string audioFilePath)
		{
			...
			Assert.AreEqual(_decodingService.BufferDataLength, QueueCapacity);
			var bytes = _decodingService.DequeueData(QueueCapacity);
			Assert.AreEqual(bytes.Length, QueueCapacity);
		}
